<template>
  ...
  <Listbox class="w-full">
    <div class="relative mt-1">
      <ListboxButton class="relative w-full cursor-default rounded-lg bg-white bg-opacity-10 py-2 pl-3 pr-10 text-left">
      ...
      <transition leave-active-class="transition duration-100 ease-in" leave-from-class="opacity-100"
        ...
        <ListboxOptions
          class="absolute z-20 max-h-60 w-full overflow-auto rounded-md bg-white bg-opacity-10 mt-1 py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
          <ListboxOption v-slot="{ active }" v-for="category in allCategories || []" :key="category.id"
            ...
            <li @click="updateSelected(category)"
              class="bg-opacity-10 relative cursor-pointer select-none bg-prime-normal hover:text-accent-normal hover:bg-prime-normal active:text-white py-2 pl-10 pr-4 rounded-lg m-1">
              {{ category.name }}
            </li>
          </ListboxOption>
        </ListboxOptions>
